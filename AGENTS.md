...
- **Three entities, one concern.** `business_hours` (weekly window, one row per
  `day_of_week`, UNIQUE), `holiday` (dated legal/administrative closure), and
  `closure` (dated local closure). `Holiday` and `Closure` are separate tables so
  the **origin** is a first-class `ClosedReason`, never a magic `type` string.
- **Not multi-tenant (signed-off).** No `tenant_id` on any table: the
  institutional calendar is one global schedule per composition-root app. This is
  an explicit decision (see `docs/ARCHITECTURE.md` §2) — do not add `tenant_id`
  casually; that would be a domain change. Consequently there are no tenant-
  isolation tests, and write/delete conditions are keyed on the row `id` (or
  `day_of_week`), which is safe for a single-tenant singleton.
- **Time encoding is minutes-from-midnight (`0..1439`)** on `open_min`/
  `close_min` — the same encoding `appointment_booking` uses for work blocks.
  It deliberately replaces the archived `business_hours` module's clock-time text
  encoding. Never introduce an `"HH:MM"` text column.
- **`NotNull` on `model.Int()`/`model.Bool()` rejects the zero value**, so the
  domain records carry it only to drive `NOT NULL` DDL; the semantic invariants
  (`ErrInvalidDay`, `ErrInvalidMinutes`, `ErrDuplicateDate`) are enforced in the
  **service** methods, the single fail-closed chokepoint every write crosses.
- **The consumer-facing contract is the `Reader` port** (`GetDayBounds`), declared
  in `interfaces.go` and satisfied structurally by `*Module`. `appointment_booking`
  imports this module only through that typed port. Resolution precedence:
  Closure > Holiday > weekly rule; a weekday with no row at all is closed by
  default.
- **`Closed` direction on events.** `CalendarChangedPayload.Closed` is `true` only
  when a change closes time — the only direction that can invalidate an existing
  reservation. When in doubt publish `Closed: true`.
- **`day_of_week` / `specific_date` uniqueness.** `day_of_week` is UNIQUE at the
  DB layer; holiday/closure same-date uniqueness is checked in the service
  (`ErrDuplicateDate`) because there is no DB constraint on `specific_date`.
